"""Tests for config/missingness.py — structural/MNAR rules."""
...
def test_apply_missingness_policy_empty():
    ...
    out, out_schema, log = _missingness.apply_missingness_policy(df, schema, [], [])
    assert len(out) == 2
...
def test_apply_structural_group():
    ...
    out, out_schema, log = _missingness.apply_missingness_policy(df, schema, groups, [])
...
def test_apply_mnar_column():
    ...
    out, out_schema, log = _missingness.apply_missingness_policy(df, schema, [], mnar)
...
def test_skipped_missing_column_does_not_crash():
    df = pd.DataFrame({"a": [1]})
    schema = {"a": ColSpec("a", "continuous")}
    mnar = [_missingness.MnarColumn(col="missing_col", reason="test")]
    out, _, log = _missingness.apply_missingness_policy(df, schema, [], mnar)
    assert len(out) == 1
