...
# --- formatting ------------------------------------------------------------
def test_p_values_drop_the_leading_zero():
    assert fm.fmt_p(0.042) == ".04"
    assert fm.fmt_p(0.0065) == ".006"


def test_a_very_small_p_is_reported_as_a_bound_not_a_number():
    assert fm.fmt_p(0.0000001) == "<.001"


def test_a_very_large_p_is_bounded_too():
    assert fm.fmt_p(0.999) == ">.99"


def test_estimates_keep_their_leading_zero():
    """Only P values drop it; applying that rule to an odds ratio is the slip."""
    assert fm.fmt_est(0.72) == "0.72"
    assert fm.fmt_est_ci(0.68, 0.61, 0.74) == "0.68 (0.61–0.74)"


def test_intervals_use_an_en_dash_without_spaces():
...
def test_native_units_keep_their_own_precision():
    """15.1 must not become 15.100, and 0.0617 must keep its digits."""
    assert fm.fmt_value(15.1, 1) == "15.1"
    assert fm.fmt_value(0.0617, 4) == "0.0617"


def test_percentages_above_one_hundred_are_shown_as_they_are():
    assert fm.fmt_pct(1.41) == "141%"


def test_yes_no_blanks_when_the_rule_could_not_be_applied():
...
# --- S1 --------------------------------------------------------------------
def test_s1_shows_the_span_as_well_as_the_ratio():
...
def test_s1_prints_the_cutpoint_with_its_direction_and_unit():
    assert _s1().loc["ADC (mean)", "Published cut-point"] == "≤ 0.72 ×10⁻³ mm²/s"


def test_all_three_stability_checks_must_hold():
    assert _s1().loc["ADC (mean)", "Cut-point locatable"] == "Yes"


def test_a_wide_bootstrap_interval_alone_makes_it_unlocatable():
    wobble = pd.DataFrame([{"col": "adc_value", "stratum": "all",
                            "cutpoint": 0.72, "ci_lo": 0.4, "ci_hi": 1.2,
                            "stability_ratio": 1.4}])
    assert _s1(wobble=wobble).loc["ADC (mean)", "Cut-point locatable"] == "No"


def test_scattered_selection_rules_alone_make_it_unlocatable():
    agreement = pd.DataFrame([{"col": "adc_value", "stratum": "all",
                               "cutoff_min": 0.5, "cutoff_max": 1.1,
                               "spread_vs_iqr": 0.8}])
    assert _s1(agreement=agreement).loc["ADC (mean)", "Cut-point locatable"] == "No"


def test_divergence_across_imputations_alone_makes_it_unlocatable():
    imputation = pd.DataFrame([{"col": "adc_value", "stratum": "all",
                                "diverges": True, "draw_min": 0.9,
                                "draw_max": 1.1}])
    assert _s1(imputation=imputation).loc["ADC (mean)",
                                          "Cut-point locatable"] == "No"


def test_s1_blanks_the_verdict_when_a_check_could_not_be_run():
    assert _s1(imputation=pd.DataFrame()).loc["ADC (mean)",
                                              "Cut-point locatable"] == fm.BLANK


# --- S2 --------------------------------------------------------------------
def test_s2_reports_both_aucs_with_intervals():
    row = _s2().loc["ADC (mean)"]
    assert row["AUC as a number"] == "0.63 (0.56–0.69)"
    assert row["AUC as yes/no"] == "0.62 (0.56–0.67)"


def test_a_small_loss_that_is_not_significant_is_acceptable():
    assert _s2().loc["ADC (mean)", "Acceptable loss"] == "Yes"


def test_a_large_loss_is_not_acceptable_even_when_not_significant():
    dich = _inputs()["dichotomy"].assign(information_retained=0.70,
                                         auc_loss_p=0.40)
    assert _s2(dichotomy=dich).loc["ADC (mean)", "Acceptable loss"] == "No"


def test_a_significant_loss_is_not_acceptable_even_when_small():
...
def test_retention_above_one_hundred_percent_is_shown_not_clipped():
...
# --- the footnotes ---------------------------------------------------------
def test_each_footnote_states_the_rule_behind_its_verdict_column():
    assert "Cut-point locatable: Yes when" in mt.s1_footnote()
    assert "Acceptable loss: Yes when" in mt.s2_footnote()


def test_the_footnotes_quote_the_thresholds_the_code_uses():
...
def test_the_s2_footnote_explains_retention_above_one_hundred():
    assert "above 100%" in mt.s2_footnote()


def test_both_footnotes_open_in_the_journal_pattern():
...
def test_a_yes_and_a_no_are_marked_so_a_graded_column_reads_at_a_glance():
    page = rh.render_table(_s1(), index_header="Measurement")
    assert 'class="yes"' in page


def test_the_table_name_and_its_footnote_both_appear():
...
def test_a_placeholder_names_its_intended_columns():
...
def test_a_missing_figure_degrades_to_a_placeholder_rather_than_breaking(tmp_path):
    page = rh.figure_section("Fig", tmp_path / "absent.png")
    assert "Not yet built" in page


def test_a_figure_is_embedded_rather_than_linked(tmp_path):
...
def test_cell_content_is_escaped():
    frame = pd.DataFrame([["<script>x</script>"]], index=["r"], columns=["c"])
    assert "<script>" not in rh.render_table(frame)


def test_the_page_can_be_pinned_for_a_reproducible_diff():
    a = rh.build([], title="T", generated_at="fixed")
    b = rh.build([], title="T", generated_at="fixed")
    assert a == b
...
def test_a_card_that_meets_everything_says_so_plainly():
    entry = _verdicts()[0]
...
def test_the_most_fundamental_failure_is_named_first():
    """With no bend at all, the later tests have nothing to be about."""
    nonlin = pd.DataFrame([{"col": "adc_value", "stratum": "all",
                            "bent_clinical": False, "lr_p": 0.50,
                            "lr_p_log": 0.77, "scales_agree": False}])
    entry = _verdicts(nonlinearity=nonlin)[0]
    assert entry["failures"][0] == "no bend"


def test_a_measurement_working_both_ways_says_so():
...
def test_a_significant_loss_names_the_number_as_the_better_form():
    _, dich, base = _verdict_inputs()
    dich = dich.assign(auc_loss=0.042, auc_loss_p=0.010)
    entry = mt.threshold_verdicts(_verdict_inputs()[0], dichotomy=dich,
                                  **base)[0]
    assert entry["works"] == "Works both ways, better as a number."


def test_a_significant_gain_names_the_rule_as_the_better_form():
    _, dich, base = _verdict_inputs()
    dich = dich.assign(auc_loss=-0.040, auc_loss_p=0.004)
    entry = mt.threshold_verdicts(_verdict_inputs()[0], dichotomy=dich,
                                  **base)[0]
    assert entry["works"] == "Works both ways, better as a yes/no."


def test_better_as_is_silent_when_the_comparison_is_absent():
...
def test_a_measurement_that_is_a_sign_not_a_dose_says_which():
    """Edema index: presence predicts, amount does not."""
    _, dich, _ = _verdict_inputs()
    dich = dich.assign(
    ...
    eligible, _, base = _verdict_inputs(presence=presence)
    entry = mt.threshold_verdicts(eligible, dichotomy=dich, **base)[0]
    assert entry["works"] == ("Works as a yes/no only — having any predicts, "
                              "how much does not.")


def test_a_null_measurement_without_a_zero_pile_gets_the_generic_line():
    _, dich, _ = _verdict_inputs()
    dich = dich.assign(
        or_per_sd=1.02, or_per_sd_lo=0.80, or_per_sd_hi=1.30, or_per_sd_p=0.87,
        or_binary=1.10, or_binary_lo=0.70, or_binary_hi=1.72)
    eligible, _, base = _verdict_inputs()
    entry = mt.threshold_verdicts(eligible, dichotomy=dich, **base)[0]
    assert entry["works"] == "Predicts neither as a number nor as a yes/no."


def test_supported_cards_sort_ahead_of_unsupported_ones():
    entries = _verdicts() + [dict(_verdicts()[0], supported=False,
                                  measurement="Zzz")]
