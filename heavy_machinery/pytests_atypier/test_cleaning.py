...
def test_number_and_table_formatting():
    ...
    out = format_table_for_csv(pd.DataFrame({"p": [0.0001, 0.05], "n": [10.0, 20.0]}))
...
def test_apply_schema(tiny_df, tiny_schema):
    ...
    assert cl._coerce_binary(pd.Series(["yes", "no", "1", "0"])).tolist() == [
...
def test_schema_coercion_collapses_numeric_and_id_formats(tmp_output):
    numeric_root = tmp_output / "numeric"
    df = pd.DataFrame({
    ...
    apply_schema(df, schema, output_root=numeric_root)
    table = pd.read_csv(numeric_root / "cleaning" / "schema_coercion.csv")
    miss = table[
    ...
    apply_schema(df, schema, output_root=id_root)
    table = pd.read_csv(id_root / "cleaning" / "schema_coercion.csv")
    id_miss = table[
...
def test_column_helpers():
    audit, cleaned = audit_duplicates(
        pd.DataFrame({"id": ["A", "A", "B"], "v": [1, 2, 3]}), ["id"])
    assert len(audit) == 2
    assert len(cleaned) == 3

    out = bin_numeric(pd.Series([25, 55, 75]), [0, 50, 70, 100],
                      labels=["<50", "50-69", "70+"])
    ...
    flag = make_missing_flag(pd.Series([1.0, np.nan, 3.0], name="age"))
    assert flag.name == "age_missing"
    assert flag.tolist() == [False, True, False]

    out = combine_categories(pd.Series(["a", "b", "c"]), {"a": "A", "b": "A"},
                             other="other")
    assert set(out.astype(str)) == {"A", "other"}

    assert abs(zscore(pd.Series([1.0, 2.0, 3.0])).mean()) < 1e-10
...
def test_the_cleaned_export_keeps_analysis_columns_and_drops_ids(
    ...
    with_derived = tiny_df.copy()
    with_derived["derived"] = 1
    schema = dict(tiny_schema)
    schema["derived"] = ColSpec("derived", "binary")
    path = write_cleaned_csv(tmp_output / "csv", with_derived, schema)
    assert "derived" in pd.read_csv(path).columns
