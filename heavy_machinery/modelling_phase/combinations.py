...
def bootstrap_best_rule(
    ...
    *,
    ...
    Note this is a different correction from the per-cut-point one in
    ``cutpoint_phase/wobble.py``: that one pays for choosing a cut-point on a
    fixed metric, this one pays for choosing which metric (or rule) to report.
...
def combination_reading_view(menu: pd.DataFrame, *, top: int | None = None) -> pd.DataFrame:
    ...
    from marker_rules import format_or_ci, format_pct_ci
